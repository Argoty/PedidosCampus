...
export class AppModule implements NestModule {
  configure(consumer: MiddlewareConsumer) {
    ...
    const serviceTokenEnv = process.env.SERVICE_TOKEN || 'internal_super_secret_token_123';
    ...
    applyProxy('/auth', process.env.AUTH_SERVICE_URL || 'http://auth-service:3001');
    applyProxy('/api/profiles', process.env.USER_SERVICE_URL || 'http://user-service:5000');
    applyProxy('/api/v1', process.env.RESTAURANT_SERVICE_URL || 'http://restaurant-service:8001');
    applyProxy('/orders', process.env.ORDER_SERVICE_URL || 'http://order-service:8002');
    applyProxy('/notifications', process.env.NOTIFICACIONES_SERVICE_URL || 'http://notificaciones-worker.local');
  }
}
