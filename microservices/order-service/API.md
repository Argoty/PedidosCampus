...
```json
{
  ...
  "estado": "pendiente|aceptado|en_camino|entregado",
  "subtotal": 25.50,
  "costoEntrega": 2.00,
  "total": 27.50,
...
```json
{
  ...
  // - pendiente -> aceptado (ya cubierto por /accept)
  // - aceptado -> en_camino
  // - en_camino -> entregado
}
```
...
**Validaciones:**
- Transiciones permitidas: `aceptado` → `en_camino` → `entregado`
...
3. NO actualizar estado (pedidos cancelados quedan en pendiente con flag)
   - O: crear nuevo estado `cancelado` en el enum
